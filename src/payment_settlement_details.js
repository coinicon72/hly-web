...
// =============================================
class PaymentSettlementDetailsPage extends React.PureComponent {
    constructor(props) {
        super(props);
        ...
        this.handleSelectClient = (e => {
            ...
            if (client != this.state.client) {
                ...
                axios.get(`${DATA_API_BASE_URL}/purchasingOrders/search/findBySupplierAndStatus?supplier=../../clients/${this.state.client.id}&status=0`)
                    .then(r => r.data._embedded.purchasingOrders)
        ...
        this.saveDocument = (async () => {
            ...
            await axios.post(`${DATA_API_BASE_URL}/paymentSettlements`, document)
                .then(resp => resp.data)
            ...
            orders.forEach(p => {
                ...
                axios.post(`${DATA_API_BASE_URL}/paymentSettlementItems`, fi)
                    .catch(e => {
                        cancel = true;
                        this.setState({ savingDocument: false })
        ...
        this.confirmDocument = () => {
            if (window.confirm("确认此结算单？")) {
                axios.patch(`${API_BASE_URL}/payment/${this.state.document.id}/confirm`)
                    .then(_ => {
    ...
    componentDidMount() {
        ...
        else //if (id > 0) 
        {
            ...
            axios.get(`${DATA_API_BASE_URL}/paymentSettlements/${id}`)
                .then(resp => resp.data)
                .then(j => {
                    ...
                    return `${DATA_API_BASE_URL}/paymentSettlements/${id}/items`
                })
                .then(url => axios.get(url))
                .then(resp => resp.data._embedded.paymentSettlementItems)
                            ...
                            {client ? (
                                <Grid>
                                    <React.Fragment>
                                        ...
                                        <Chip label={`${client.paymentPolicy}, ${client.paymentPeriod}天`} className={classes.chip} />
                                    </React.Fragment>
                                </Grid>
                            ) : null}
